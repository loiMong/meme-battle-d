...
function adminRenderTaskStats(){
  const box = $("admin-taskstats");
  if(!box) return;
  const db = adminState.last?.db || {};
  const ok = db && db.ok ? "OK" : "OFF";
  const cnt = Number(db.globalTaskStatsCount || 0);
  box.innerHTML = `
    <div class="adminCard">
      <div class="adminCardTitle">DB: <b>${h(ok)}</b></div>
      <div class="muted">global_task_stats записей: <b>${h(cnt)}</b></div>
      <div class="muted" style="margin-top:8px">Пока без логики — тут будут агрегаты по заданиям.</div>
    </div>
  `;
}
...
function adminSetTab(tab){
  ...
  ["overview","taskstats","roomstats","topics","rooms","sandbox","logs"].forEach(t=>{
...
async function adminLogin(){
  ...
  if(!adminState.token){
    adminSetStatus("Нужен пароль/токен (см. ADMIN_PASSWORD в Secrets на сервере).");
    return;
  }
  const data = await adminApi("/api/admin/overview");
  if(!data.ok){
