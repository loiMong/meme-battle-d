...
// [ANCHOR] MB:F:VOTING_FINISHED — show next round button to players + enable host continuation
socket.on("voting-finished", (p)=>{
  try{ pushDebug("voting-finished", p); }catch(e){}
  const roomCode = String(p?.roomCode||"").toUpperCase();
  // Host: enable next round button when vote is finished
  if(roomCode && roomCode === String(currentRoom||"").toUpperCase()){
    try{ $("host-next-round").disabled = false; }catch(e){}
  }

  // Player: show "next round" confirm
  if(playerState.joined && roomCode === String(playerState.roomCode||"").toUpperCase()){
    try{
      $("player-next-wrap")?.classList.remove("hidden");
      const btn = $("player-ready-next");
      const msg = $("player-ready-next-msg");
      if(btn){
        btn.disabled = false;
        btn.onclick = ()=>{
          btn.disabled = true;
          socket.emit("player-ready-next", { roomCode: playerState.roomCode }, (res)=>{
            try{ pushDebug("player-ready-next", res); }catch(e){}
            if(!res?.ok){
              btn.disabled = false;
...
// When voting starts — hide next-round UI
socket.on("voting-started", (p)=>{
  try{
    if(playerState.joined && String(p?.roomCode||"").toUpperCase() === String(playerState.roomCode||"").toUpperCase()){
      $("player-next-wrap")?.classList.add("hidden");
      $("player-ready-next-msg")?.classList.add("hidden");
      const btn = $("player-ready-next"); if(btn) btn.disabled = false;
    }
  }catch(e){}
});

// Host: if everyone pressed next, auto-enable next button (visual hint)
socket.on("all-ready-next", (p)=>{
  try{ pushDebug("all-ready-next", p); }catch(e){}
  const roomCode = String(p?.roomCode||"").toUpperCase();
  if(roomCode && roomCode === String(currentRoom||"").toUpperCase()){
    try{ $("host-next-round").disabled = false; }catch(e){}
  }
