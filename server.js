...
// [ANCHOR] MB:S:ADMIN_AUTH — admin password/token
const ADMIN_PASSWORD = String(process.env.ADMIN_PASSWORD || process.env.ADMIN_TOKEN || "dev").trim(); // Join: roomCode=ADMIN, nickname=password
const ADMIN_TOKEN = String(process.env.ADMIN_TOKEN || process.env.ADMIN_PASSWORD || "dev").trim(); // legacy (admin mode screen)
...
// [ANCHOR] MB:S:STATS_DB — minimal JSON-backed store (no external deps)
// Stores: global_task_stats (aggregates). Room stats are ephemeral with TTL (10h).
const DB_FILE = path.join(__dirname, "mb_stats.json");
let DB_OK = true;

// global_task_stats: { [taskKey]: { uses, likes, dislikes, wins, updatedAt } }
let globalTaskStats = Object.create(null);

function dbLoad(){
  try{
    const raw = fs.readFileSync(DB_FILE, "utf-8");
    const j = JSON.parse(raw);
    const g = j && typeof j.global_task_stats === "object" ? j.global_task_stats : null;
    globalTaskStats = g && g !== null ? g : Object.create(null);
    DB_OK = true;
  }catch(e){
    // file might not exist yet — that's OK
    globalTaskStats = Object.create(null);
    DB_OK = true;
  }
}
function dbSave(){
  try{
    fs.writeFileSync(DB_FILE, JSON.stringify({ global_task_stats: globalTaskStats, updatedAt: Date.now() }, null, 2));
    DB_OK = true;
  }catch(e){
    DB_OK = false;
  }
...
function globalTaskStatsCount(){
  try{ return Object.keys(globalTaskStats || {}).length; }catch(e){ return 0; }
}

// room_stats: TTL 10 hours (in-memory only)
const ROOM_STATS_TTL_MS = 10 * 60 * 60 * 1000;
const roomStatsStore = Object.create(null);

function roomStatsSet(roomCode, patch){
  const code = String(roomCode||"").toUpperCase();
  if(!code) return;
  const prev = roomStatsStore[code] || {};
  roomStatsStore[code] = Object.assign({}, prev, patch||{}, { updatedAt: Date.now() });
}
function roomStatsGet(roomCode){
  const code = String(roomCode||"").toUpperCase();
  const v = roomStatsStore[code];
  if(!v) return null;
  if(Date.now() - Number(v.updatedAt||0) > ROOM_STATS_TTL_MS){
    delete roomStatsStore[code];
    return null;
  }
  return v;
}
function roomStatsCleanup(){
  const now = Date.now();
  for(const [code,v] of Object.entries(roomStatsStore)){
    if(!v){ delete roomStatsStore[code]; continue; }
    if(now - Number(v.updatedAt||0) > ROOM_STATS_TTL_MS) delete roomStatsStore[code];
  }
}
const __roomStatsTimer = setInterval(roomStatsCleanup, 10*60*1000);
try{ __roomStatsTimer.unref(); }catch(e){}
...
function requireAdmin(req, res){
  ...
  if(!t || (t !== ADMIN_TOKEN && t !== ADMIN_PASSWORD)){
